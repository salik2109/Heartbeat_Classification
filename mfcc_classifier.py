...
def load_audio_data(folder, file_names, duration=3, sr=SAMPLE_RATE):
...
def load_data_and_split():
    ...
    if isfile('x_data.npy') == 0 and isfile('y_data.npy') == 0:
        ...
        B_folder = INPUT_DIR + 'PSM/'

        B_normal_files = fnmatch.filter(os.listdir(INPUT_DIR + 'PSM/'), '*.wav')  # include noisy files
        B_normal_sounds = load_audio_data(folder=B_folder, file_names=B_normal_files, duration=MAX_SOUND_CLIP_DURATION)
        B_normal_labels = [1 for items in B_normal_sounds]


        print("loaded PSM dataset")

        x_data = np.concatenate((A_normal_sounds, B_normal_sounds))

        y_data = np.concatenate((A_normal_labels, B_normal_labels))
